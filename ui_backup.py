...
class JarvisUI:
    ...
    def _draw_left_panel(self, c):
        ...
        cards = [
            ("time", 0.16, "TIME", C_GOLD),
            ("weather", 0.18, "WEATHER", C_BLUE),
            ("system", 0.28, "SYSTEM STATUS", C_PRI),
            ("tasks", 0.22, "ACTIVE TASKS", C_GREEN),
            ("media", 0.16, "MEDIA CONTROL", "#cc2255"),
        ]
        ...
        for (section, _, title, accent), ph in zip(cards, heights):
            ...
            if section == "time":
                c.create_text(section_x+section_pad, current_y+42, text=time.strftime("%H:%M"),
                              fill=muted_primary, font=font_display(28 if focus_boost > 0.08 else 26), anchor="w")
                c.create_text(section_x+section_pad+80, current_y+42, text=time.strftime(":%S"),
                              fill=muted_label, font=font_body_bold(11), anchor="w")
                c.create_text(section_x+section_pad, current_y+68, text=time.strftime("%d %B %Y").upper(),
                              fill=muted_gold, font=font_body_bold(10), anchor="w")
                c.create_text(section_x+section_pad, current_y+84, text=time.strftime("%A").upper(),
                              fill=muted_text, font=font_body(9), anchor="w")

            elif section == "weather":
                c.create_text(section_x+section_pad, current_y+42, text=self._weather_card["primary"],
                              fill=muted_primary, font=font_display(22 if focus_boost > 0.08 else 20), anchor="w")
                c.create_text(section_x+section_pad, current_y+62, text=self._weather_card["city"].upper(),
                ...
                dummy_tasks = [
                    ...
                    "[ ] Yapay zeka bellek senkronizasyonu",
                    "[ ] E-posta kontrolleri",
                    "[ ] Geliştirici modunu aktif et"
                ]
                for line in dummy_tasks:
                    color = muted_green if "[✓]" in line else muted_text
                    c.create_text(section_x+section_pad, hy, text=line, fill=color, font=font_body(10), anchor="w")
                    hy += 20
                    
            elif section == "media":
                my = current_y + 45
                c.create_text(section_x+section_pad, my, text="▶ NO MEDIA PLAYING", fill=muted_label, font=font_body_bold(10), anchor="w")
                c.create_text(section_x+section_pad, my + 18, text="Spotify or YouTube is idle", fill=muted_text, font=font_body(9), anchor="w")
                # draw a fake progress bar
                self._bar(c, section_x+section_pad, my + 38, section_bw, 4, 0, muted_primary)
