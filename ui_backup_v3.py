...
class JarvisUI:
    def __init__(self, start_hidden=False):
        ...
        self._ai_workflow_tasks = deque([
            ...
            ("[ WAITING ]", "Background Telemetry Scan", C_DIM),
        ], maxlen=4)
    ...
    def _draw_sleep_button(self):
        ...
        BW, BH = 126, 36
        c.delete("all")
        col = C_GOLD if self._sleep_hover else C_GREEN
        bg = "#0a1a12" if self._sleep_hover else C_BG
    ...
    def _place_layout_widgets(self):
        ...
        panel_h = min(340, self.H - HDR_H - FOOTER_H - 40)
        panel_x = 14
        panel_y = HDR_H + 10
    ...
    def _draw_info_card(self, c, x0, y0, pw, ph, title, accent=C_PRI):
        ...
        if focus > 0.08:
            ...
            r = int(0 + 255 * max(0, pulse_sin))
            g = int(212 + 43 * math.cos(self.tick * 0.25))
            b = int(192 + 63 * math.sin(self.tick * 0.25))
    ...
    def _draw_notifications(self, c):
        ...
        for notif in self._notifications:
            ...
            # Card background
            nw, nh = 320, 40
            c.create_rectangle(nx - nw, ny, nx, ny + nh, fill="#030d0d", outline="")
    ...
    def _draw_left_panel(self, c):
        ...
        for (section, _, title, accent), ph in zip(cards, heights):
            ...
            if section == "time":
                ...
                c.create_text(section_x+section_pad+90, ry1, text=cond_s.title(),
                ...
                c.create_line(section_x+section_pad, div_y, section_x+section_pw-section_pad, div_y, fill="#0f2a28")

                # Row 4: Suggestion
                ry4 = current_y + int(ph * 0.81)
                ...
                c.create_line(section_x+section_pad, net_div, section_x+section_pw-section_pad, net_div, fill="#0f2a28")
    ...
    def _draw(self):
        ...
        c.create_text(W-140, 46, text=f"{batt_icon} {batt:.0f}%  ·  {MODEL_BADGE}",
                      fill=C_DIM, font=font_body(9), anchor="e")

        # ── PENCERE KONTROL BUTONLARI ──
        c.create_rectangle(W-125, 6, W-90, 32, fill="#031414", outline=C_MID)
        c.create_text(W-107, 18, text="_", fill=C_TEXT, font=font_display(12))
        
        c.create_rectangle(W-85, 6, W-50, 32, fill="#031414", outline=C_MID)
