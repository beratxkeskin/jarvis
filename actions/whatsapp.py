"""
WhatsApp mesaj gonderme - Windows Desktop veya WhatsApp Web ile calisir.
"""
...
from actions.windows_utils import copy_to_clipboard, open_url, press_key, send_hotkey
...
def _auto_send() -> tuple[bool, str]:
    if press_key("enter", delay=AUTO_SEND_DELAY_SECONDS):
        return True, "Enter gonderildi."
    return False, "PyAutoGUI ile Enter gonderilemedi."
...
def _open_whatsapp_desktop_via_scheme(phone_number: str, message: str) -> tuple[bool, str]:
    ...
    try:
        open_url(f"whatsapp://send?phone={phone_number}&text={encoded_message}")
...
def _open_whatsapp_desktop_by_name(recipient_name: str, message: str, send_now: bool) -> tuple[bool, str]:
    try:
        open_url("whatsapp:")
        copy_to_clipboard(recipient_name.strip())
    except Exception as exc:
        return False, f"WhatsApp Desktop acilamadi: {exc}"

    if not send_hotkey("ctrl", "f", delay=1.1):
        return False, "PyAutoGUI ile WhatsApp aramasi baslatilamadi."
    send_hotkey("ctrl", "a", delay=0.2)
    send_hotkey("ctrl", "v", delay=0.1)
    press_key("enter", delay=1.0)

    try:
        copy_to_clipboard(message.strip())
    except Exception as exc:
        return False, f"Mesaj panoya kopyalanamadi: {exc}"
    if not send_hotkey("ctrl", "v", delay=0.7):
        return False, "PyAutoGUI ile mesaj yapistirilamadi."
    if send_now:
...
def send_whatsapp_message(
    ...
    if not normalized_phone:
        if resolved_name:
            return f"'{resolved_name}' icin kayitli bir telefon numarasi bulamadim. Once kisiyi numarasiyla kaydet."
