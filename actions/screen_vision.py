...
def _capture_screen() -> tuple[bool, str, dict | None]:
    try:
        ...
        return False, f"Windows ekran goruntusu alinamadi: {exc}", None
...
def _vision_prompt(query: str, owner_name: str, window_title: str) -> str:
    ...
    return (
        "Sen Windows uzerinde JARVIS icin ekran analizi yapan 'Contextual Vision' modulunun gozlerisin.\n"
        "Asagidaki ekran goruntusu kullanicinin anlik masaustunden (veya aktif penceresinden) alindi.\n"
        f"Pencere baglami: {label}\n\n"
        "Gorevlerin:\n"
        "1. KULLANICI SORUSUNA ODAKLAN: Kullanici sorusunu bu goruntuye gore dogrudan ve kesin bir dille cevapla.\n"
        "2. KOD INCELEMESI (Zero-Copy): Eger kullanici bir koddan, hatadan veya mantiksal problemden bahsediyorsa ve ekranda bir IDE (VS Code vb.) aciksa, kodu doğrudan resimden oku ve satir/satir analiz ederek cozumu soyle. Kodu kopyalamasini ISTEME.\n"
        "3. ARAYUZ REHBERI (UI Navigation): Eger kullanici bir butonu, menuyu veya arayuz elemanini soruyorsa, ekranin neresinde oldugunu (ornegin 'sol ust kosede', 'mavi renkli') tarif et.\n"
        "4. DETAY: Ekranda gozuken yazilari, hata loglarini (Exception/Traceback) kendi metninmis gibi oku ve acikla.\n"
        "5. GUVENLIK: Ekranda sifreler, ozel banka bilgileri veya asiri kisisel kimlik bilgileri gorursen, spesifik detaylari sansurleyerek analiz yap.\n\n"
        f"Kullanici sorusu: {user_query}\n\n"
        "Yaniti Turkce, profesyonel bir asistan tonuyla ver."
    )
...
def _friendly_vision_error(exc: Exception) -> str:
    message = str(exc or "").lower()
    if any(marker in message for marker in ("quota", "rate limit", "resource exhausted", "too many requests", "billing")):
...
def analyze_screen(query: str, target: str = "active_window") -> str:
    ...
    if target not in {"active_window", "screen", "desktop"}:
        return "Screen Vision v1 Windows'ta aktif ekran analizini destekliyor."
    ...
    try:
        if not image_path.exists() or image_path.stat().st_size <= 0:
            return "Ekran goruntusu bos geldi. Windows gizlilik ve ekran yakalama izinlerini kontrol et."
        if _image_looks_blank(image_path):
            return "Ekran goruntusu siyah veya bos gorunuyor. Korumali bir pencere acik olabilir."
        try:
